...
app.post('/pessoas/insert', urlencodedParser, function(req,  res){
    try {
        ...
        if(req.body.idade) objJSON.idade = Number(req.body.codigo);  
...
app.get('/pessoas/find', urlencodedParser), function(req, res){
    try {
        ...
        if(req.query.idade) objJSON.idade = Number(req.query.codigo);  
        else objJSON.idade = 18;
...
app.post('/pessoas/update/:codigo', urlencodedParser, function(req,  res){
    try {
        let codigo = 0;
        if (req.params.codigo) codigo = Number(req.params.codigo);
        let objJSON = {};
